...
parameters {
  ...
  vector[half_NJ] r_ln_phi_amb;  // pond-level random effects
...
transformed parameters {
  ...
  real ln_phi_amb = log(phi_amb);
  real ln_phi_war = log(phi_war);
  ...
  vector[NJ] r_phi = append_row(exp(ln_phi_amb + r_ln_phi_amb), exp(ln_phi_war + r_ln_phi_war));
}
model {
  vector[N] mu;
  ...
  r_ln_phi_amb ~ normal(0, hyper_sd_ln_phi);
  ...
  r_ln_phi_war ~ normal(0, hyper_sd_ln_phi);

  hyper_sd_ln_ka ~ gamma(2, 0.1);
  hyper_sd_logit_ke ~ gamma(2, 0.1);
  hyper_sd_ln_phi ~ gamma(2, 0.1);
